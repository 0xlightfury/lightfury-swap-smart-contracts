...
contract FURYPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 furyAtLastUserAction; // keep track of fury deposited at the last user action.
    ...
    event NewVFuryContract(address VFury);
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(furyPoolPID, balance, address(0));
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the fury owner address or the operator address.
     */
    modifier onlyOperatorOrFuryOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or fury owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onFuryPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                if (!freeOverdueFeeUsers[_user] && ((user.lockEndTime + UNLOCK_FREE_DURATION) < block.timestamp)) {
                    ...
                    IERC20(token).safeTransfer(treasury, currentOverdueFee);
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.furyAtLastUserAction;
                ...
                if (currentPerformanceFee > 0) {
                    IERC20(token).safeTransfer(treasury, currentPerformanceFee);
    ...
    /**
     * @notice Unlock user fury funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrFuryOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Fury Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in FURY)
    ...
    function depositOperation(
        ...
        if (VFury != address(0)) {
            ...
            IVFury(VFury).deposit(_user, _amount, _lockDuration, referrer);
        }
        ...
        if (totalShares == 0) {
            ...
            IERC20(token).safeTransfer(treasury, stockAmount);
        }
        // Update user share.
        ...
        user.furyAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Fury Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VFury != address(0)) {
            IVFury(VFury).withdraw(msg.sender);
        }
        ...
        if (!freeWithdrawFeeUsers[msg.sender] && (block.timestamp < user.lastDepositedTime + withdrawFeePeriod)) {
            ...
            IERC20(token).safeTransfer(treasury, currentWithdrawFee);
            currentAmount -= currentWithdrawFee;
        }

        IERC20(token).safeTransfer(msg.sender, currentAmount);

        address referrer = IReferral(referral).getReferrer(msg.sender);
        if (referrer != address(0)) {
            uint256 rFee = IReferral(referral).rFee();
            uint256 refAmount = (currentAmount * rFee) / 100;
            IFuryToken(token).mint(referral, refAmount);
            IReferral(referral).notifyReward(referrer, refAmount);
        }

        if (user.shares > 0) {
            user.furyAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.furyAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending FURY tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingFury = masterchefV2.pendingFury(furyPoolPID, address(this));
        if (pendingFury > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(furyPoolPID, 0);
    ...
    /**
     * @notice Set VFury Contract address
     * @dev Callable by the contract admin.
     */
    function setVFuryContract(address _VFury) external onlyAdmin {
        require(_VFury != address(0), "Cannot be zero address");
        VFury = _VFury;
        emit NewVFuryContract(VFury);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Fury Pool
     */
    function inCaseTokensGetStuck(address _token) external onlyAdmin {
        require(_token != token, "Token cannot be same as deposit token");
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingFuryRewards();
     ...
     * @return Returns total pending fury rewards
     */
    function calculateTotalPendingFuryRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingFury(furyPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingFuryRewards()) * (1e18)) / totalShares);
    }
    ...
    function balanceOf() public view returns (uint256) {
        return IERC20(token).balanceOf(address(this)) + totalBoostDebt;
    }
