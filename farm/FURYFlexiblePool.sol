...
contract FuryFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 furyAtLastUserAction; // keeps track of fury deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    address public referral;
    address public immutable token; // Fury token
    IFuryPool public immutable furyPool; //Fury pool
    ...
    //When call furypool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositFury(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_furyPool), type(uint256).max);
    ...
    /**
     * @notice Deposits funds into the Fury Flexible Pool.
    ...
    function deposit(uint256 _amount, address _referrer) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    furyPool.withdrawByAmount(withdrawAmount);
        ...
        emit DepositFury(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Fury Flexible Pool
    ...
    /**
     * @notice Withdraws from Fury Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking fury");
        staking = false;
        furyPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Fury Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in FuryPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = furyPool.userInfo(address(this));
        uint256 pricePerFullShare = furyPool.getPricePerFullShare();

        return IERC20(token).balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into FuryPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            furyPool.deposit(bal, 0, address(0));
        }
    }
}