...
/** @title LightFurySwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract LightFurySwapLottery is ReentrancyGuard, ILightFurySwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInFury = 50 ether;
    uint256 public minPriceTicketInFury = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInFury;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] furyPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInFury;
        uint32 finalNumber;
    }
     ...
     * @param _furyTokenAddress: address of the FURY token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _furyTokenAddress, address _randomGeneratorAddress, address _initializer) {
        furyToken = IERC20(_furyTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of FURY to this contract
        uint256 amountFuryToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInFury,
            _ticketNumbers.length
        );

        // Transfer fury tokens to this contract
        furyToken.safeTransferFrom(address(msg.sender), address(this), amountFuryToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInFury += amountFuryToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInFuryToTransfer
        uint256 rewardInFuryToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInFuryToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        furyToken.safeTransfer(msg.sender, rewardInFuryToTransfer);

        emit TicketsClaim(msg.sender, rewardInFuryToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInFury) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in FURY for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].furyPerBracket[j] =
                ...
                // A. No FURY to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].furyPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInFury - amountToShareToWinners);

        // Transfer FURY to treasury address
        furyToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        furyToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInFury += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInFury >= minPriceTicketInFury) && (_priceTicketInFury <= maxPriceTicketInFury),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInFury: _priceTicketInFury,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            furyPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInFury: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(furyToken), "Cannot be FURY token");
    ...
    /**
     * @notice Set FURY price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInFury: minimum price of a ticket in FURY
     * @param _maxPriceTicketInFury: maximum price of a ticket in FURY
     */
    function setMinAndMaxTicketPriceInFury(uint256 _minPriceTicketInFury, uint256 _maxPriceTicketInFury)
        external
        onlyOwner
    {
        require(_minPriceTicketInFury <= _maxPriceTicketInFury, "minPrice must be < maxPrice");

        minPriceTicketInFury = _minPriceTicketInFury;
        maxPriceTicketInFury = _maxPriceTicketInFury;
    }
     ...
     * @param _priceTicket price of a ticket (in FURY)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].furyPerBracket[_bracket];
        } else {
            return 0;
        }
    }
